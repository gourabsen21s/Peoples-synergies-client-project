  ...
  const formTypeToComponent: { [type in ShowForm]: () => React.JSX.Element } = {
    workExperiences: () => (
      <ResumePDFWorkExperience
        ...
        showBulletPoints={showBulletPoints["workExperiences"] as any}
        isPDF={isPDF}
      />
    ),
    educations: () => (
        ...
        >
          ...
          {settings.template === "modern" && (
            ...
            >
              ...
              {showFormsOrder.map((form, idx) => {
                const Component = formTypeToComponent[form];
                return (
                  <View key={form} style={{ marginTop: idx === 0 ? 0 : spacing[6] }}>
                    <Component />
                    <View style={{ borderBottom: 1, borderBottomColor: "#eee", marginTop: spacing[2] }} />
                  </View>
                );
              })}
            </View>
          ...
          {settings.template === "executive" && (
            ...
            >
              ...
              {showFormsOrder.map((form) => {
                const Component = formTypeToComponent[form];
                return (
                  <View key={form} style={{ marginTop: spacing[6] }}>
                    <Component />
                  </View>
                );
              })}
