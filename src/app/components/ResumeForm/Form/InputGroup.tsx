    ...
    <InputGroupWrapper label={label} className={wrapperClassName}>
      <div className="flex items-center gap-2 mb-2">
        <button
          ...
          onClick={() => {
            const suggestions = [
              "Led a team of 5 developers to deliver a $2M project 2 weeks ahead of schedule.",
              "Optimized database queries reducing application latency by 40% and improving user experience.",
              "Implemented automated CI/CD pipelines increasing deployment frequency by 300%.",
              "Collaborated with cross-functional teams to define product roadmap and technical strategy.",
              "Mentored junior engineers resulting in a 25% increase in team productivity and code quality."
            ];
            const random = suggestions[Math.floor(Math.random() * suggestions.length)];
            if (!bulletListStrings.includes(random)) {
              onChange(name, [...bulletListStrings, random]);
            }
          }}
          className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-primary/10 text-primary text-[10px] font-bold hover:bg-primary/20 transition-colors uppercase tracking-wider"
          title="AI Suggestion"
        >
